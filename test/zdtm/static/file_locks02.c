...
static int check_file_lock(pid_t pid, int fd, char *expected_type,
			   char *expected_option)
{
	...
	test_msg("check_file_lock: (pid %d) expecting fd %d type %s option %s\n",
		 pid, fd, expected_type, expected_option);
...
int main(int argc, char **argv)
{
	...
	if (check_file_lock(getpid(), fd, "ADVISORY", "WRITE")) {
