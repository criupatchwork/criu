...
class zdtm_test:
	...
	def stop(self):
		...
		if b'PASS' not in list(map(lambda s: s.strip(), res.split())):
...
class criu:
	...
	def dump(self, action, opts = []):
		...
		if self.__check_only:
			...
			os.chmod(self.__ddir(True), 0o777)
		...
		if pid != 0:
			...
			if status != 0:
				...
				if self.__file_report:
					...
					print(testline, file=self.__file_report)
					print(unicode(yaml.dump(details, explicit_start=True, explicit_end=True, default_style='|')), file=self.__file_report)
				if sub['log']:
